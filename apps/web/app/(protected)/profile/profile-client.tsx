...
export type ProfileInitialData = {
  ...
  passkeys: PasskeyCredential[] | null
  emailPreferences: EmailNotificationPreferences | null
}

export function ProfileClient({ initialData }: { initialData?: ProfileInitialData }) {
  return (
...
function ProfileContent({ initialData }: { initialData?: ProfileInitialData }) {
  const hasCompleteInitialData = initialData !== undefined
    && initialData.profile !== null
    && initialData.passkeys !== null
    && initialData.emailPreferences !== null
  const [isEditing, setIsEditing] = useState(Boolean(initialData?.profile && (!initialData.profile.nickname || initialData.profile.instruments.length === 0)))
  const [loading, setLoading] = useState(!hasCompleteInitialData)
  const [userData, setUserData] = useState<UserData | null>(initialData?.profile ?? null)
  const [profileError, setProfileError] = useState(initialData?.profile === null)
  const [profileRetryKey, setProfileRetryKey] = useState(0)
  const [isPasskeyProcessing, setIsPasskeyProcessing] = useState(false)
  const [isPasskeyLoading, setIsPasskeyLoading] = useState(false)
  const [passkeys, setPasskeys] = useState<PasskeyCredential[]>(initialData?.passkeys ?? [])
  ...
  const [emailNotificationPreferences, setEmailNotificationPreferences] = useState<EmailNotificationPreferences | null>(initialData?.emailPreferences ?? null)
  const [emailNotificationError, setEmailNotificationError] = useState(initialData?.emailPreferences === null)
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (hasCompleteInitialData && profileRetryKey === 0) return
      try {
    ...
    init()
  }, [authLoading, user, router, fetchPasskeys, fetchEmailNotificationPreferences, hasCompleteInitialData, pathname, profileRetryKey, searchParams])
