  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-4xl space-y-4 p-4 pt-0">
        ...
        <Card>
          <CardHeader><CardTitle>ホール抽選</CardTitle></CardHeader>
          <CardContent className="space-y-3 text-sm leading-relaxed">
            <ol className="list-decimal space-y-2 pl-5">
              <li>「抽選」のホール抽選で対象期間と共通利用時間を確認し、所属バンドを選びます。抽選ごとに本バンドまたは自由バンドのどちらかが対象となり、所属メンバーが代表して申し込めます。管理者は対象区分に一致する非所属の有効なバンドの代理申込・締切前の取消もできます。</li>
              <li>第1希望の開始日時を入力します。第2・第3希望は任意で、終了日時は抽選の共通利用時間から自動計算されます。</li>
              <li>期間全体につき1バンド1申込・最大1枠の当選です。締切前なら取消後に再申込できます。</li>
            </ol>
            <p>各日6:00〜23:00、日またぎ不可。同一日時の重複はできませんが、希望同士が部分的に重なっていても申し込めます。抽選の利用時間は10〜240分で、14日先までの申込制限はありません。</p>
            ...
            <p>抽選作成から抽選完了まで、対象期間の通常予約は制限されます（既存の管理者例外を除く）。処理失敗中も保護を継続し、中止または完了後に空きを開放します。既存予約は取り消しません。</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4 text-sm leading-relaxed text-foreground">
            ...
            <p>
              同じメンバーが参加する受付済みのホール予約・確定済みの外部予約や、同じ抽選ですでに当選した予約と時間が重ならない候補だけを使います。実際の割当時間が予約上限を超える候補も使用しません。
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm leading-relaxed text-foreground">
            ...
            <p>
              公平性スコアは、個人は個人ID、バンドはグループIDごとに外部利用時間を集計して計算します。キャンセル済みの予約も集計に含み、予約時間を変更した場合は変更後の分数を使います。
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm leading-relaxed text-foreground">
            <p>「抽選」では、外部の抽選対象が横方向に並び、その下に各時間枠への申込がすべて表示されます。</p>
            <ul className="list-disc space-y-1 pl-5">
              <li>自分または所属バンドに関係する申込は、濃い枠線で表示されます。</li>
              <li>抽選前は、既存予約を除いた空きを競合申込へ公平配分し、優先区分ごとに時間の余裕と公平性から計算したウェイトで処理順を複数回抽選した、おおよその当選確率が表示されます。希望時間より短い割当でも、10分以上なら当選として数えます。</li>
